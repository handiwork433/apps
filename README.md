...
2. Скопируйте готовый `server/.env.example` в `server/.env` и заполните значения:
   ```env
...
### Автоматическая установка на Ubuntu 22.04 LTS

Если нужно быстро развернуть сервер прямо на удалённой Ubuntu 22.04, используйте встроенный установщик `server/install.sh`. Он установит Node.js 18 LTS, PM2, скопирует `.env`, подтянет зависимости и подготовит systemd-unit. После запуска останется только прописать реальные значения токенов.

1. Клонируйте репозиторий на сервер (пример с домашней директорией пользователя `ubuntu`):
   ```bash
   cd ~
   git clone https://github.com/username/telegram-text-app.git
   cd telegram-text-app
   ```
2. Запустите установщик от имени root (или через `sudo`):
   ```bash
   sudo bash server/install.sh
   ```
   Скрипт выполнит следующие шаги:
   - обновит apt-кеш и установит `curl`, `gnupg`, `git` и другие зависимости;
   - добавит репозиторий NodeSource и поставит Node.js 18 (если он ещё не установлен);
   - установит PM2 глобально (на будущее);
   - создаст `server/.env` из шаблона, если файла нет;
   - установит npm-зависимости в `server/` под вашим пользователем;
   - создаст systemd-сервис `telegram-text-app` и перезагрузит список юнитов;
   - автоматически включит и перезапустит сервис, если в `.env` уже прописан реальный `TELEGRAM_BOT_TOKEN`.
3. Отредактируйте файл `server/.env`, если ещё не сделали этого:
   ```bash
   nano server/.env
   ```
   Минимально необходимо заменить `__PASTE_TELEGRAM_BOT_TOKEN__` на токен бота и, при необходимости, скорректировать `TELEGRAM_BOT_LINK`, `PORT`, `SUBSCRIPTION_PRICE` и другие параметры.
4. После правок перезапустите сервис:
   ```bash
   sudo systemctl restart telegram-text-app
   sudo systemctl status telegram-text-app
   ```
   Логи можно посмотреть командой `journalctl -u telegram-text-app -f`.
5. Убедитесь, что порт открыт в брандмауэре (пример для UFW):
   ```bash
   sudo ufw allow 3000/tcp
   ```

Если по каким-то причинам нужно выполнить установку вручную, см. следующий раздел.

### Ручное развёртывание на Ubuntu 22.04 LTS

Ниже приведён пример пошагового развёртывания без установщика. При необходимости адаптируйте команды под свою конфигурацию (например, другой пользователь, порт или домен).
...
> Если `PAYMENT_PROVIDER_TOKEN` не задан, команда `/subscribe` просто сообщит, что платёжный провайдер не настроен. Продлевать подписку можно командой `/activate <секрет> [дней]`.
